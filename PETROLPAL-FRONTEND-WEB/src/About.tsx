...
const About = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      // Type guard to ensure target is an HTMLElement with id property
      if (target && 'id' in target && typeof (target as HTMLElement).id === 'string') {
        const element = target as HTMLElement;
        
        if (showDownloadModal && element.id === 'downloadModalOverlay') {
          setShowDownloadModal(false);
        }
        if (showContactModal && element.id === 'contactModalOverlay') {
